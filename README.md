...
└── MLProject/
    ...
    │   ├── y_train.csv
    │   └── y_test.csv
    ├── docker-info/                   # Docker deployment info (by CI)
    │   └── Tautan_ke_Docker_Hub.txt
    ├── mlruns/                        # MLflow tracking data (by CI)
    ├── model/                         # Exported model (by CI)
    ├── conda.yaml                     # Environment dependencies
    ├── MLProject                      # MLflow project configuration
    ├── modelling.py                   # Training script
    ├── Dockerfile                     # Generated by CI
    └── README.md
```
...
- 📤 Push ke branch `main` (pada folder MLProject atau workflow file)
